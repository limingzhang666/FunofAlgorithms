# -*- coding: utf-8 -*-
"""
@Time : 2020/6/30 17:40
@Author : liming
@Email : laimingcham@163.com
@File : pourWater.py
@Project : FunofAlgorithms
@Desc:  三个水桶等分8升水问题python实现--穷举、简单状态转移与递归
"""
from collections import deque

"""
https://www.jianshu.com/p/46f6aa196e1ex
"""

# TODO  关于其中的yield 后面看看可以如果去替换掉，以及 比较不用yield的效率，总感觉  nextStateLawful 被重复执行了好多次
class Solution:

    def nextStateLawful(self, current_state, bucket_volume):
        # 通过列表推导式获得下一动作的二元组构成的列表，由（倒出水的容器编号，倒入水的容器编号）组成
        # 1.倒入倒出不能为同一个2.倒出的捅中必须有水3.倒入的桶中不能为满 的条件判断是否合法
        next_action = [
            (from_, to_)
            for from_ in range(3) for to_ in range(3)
            if from_ != to_
               and current_state[from_] > 0
               and current_state[to_] < bucket_volume[to_]
        ]

        # 二重循环得到下一步的所有可能动作，然后通过
        for from_, to_ in next_action:
            next_state = list(current_state)
            # 当 导入量+当前量> 桶的容量，则 from减少的量= 最大容量-当前量，to=最大容量
            if current_state[from_] + current_state[to_] > bucket_volume[to_]:
                next_state[from_] -= bucket_volume[to_] - current_state[to_]
                next_state[to_] = bucket_volume[to_]
            else:
                # 当 导入量 +当前量<=桶的容量，则from 可以全部倒出，to 可以全部接受
                next_state[from_] = 0
                next_state[to_] = current_state[to_] + current_state[from_]
            # 再由 所有可能的合法动作 得出所有的下一状态，通过yield产生提供给其他函数 调用
            yield next_state



    # 记录调试的变量：num表示总共实现方法数，record_list记录所有实现路径

    def searchResult(self, record, bucket_volume=[3, 5, 8], final_bucket_state=[0, 4, 4]):
        global num, record_list
        current_state = record[-1]
        # 由record的末尾元素得到当前水桶状态
        next_state = self.nextStateLawful(current_state, bucket_volume)
        # 得到关于当前状态的下一状态 的可迭代生成器，拱下一步循环使用
        for state in next_state:
            # 遍历所有可能的下一状态
            if state not in record:
                # 保证当前状态没在以前出现过。 如果状态已经出现还进行搜索就会形成状态 环路，陷入死循环
                record.append(state)
                # 添加新的状态 到列表中
                if state == final_bucket_state:
                    print(record)
                    # 打印出可行方案
                    # record_list.append(record)这样使用错误，导致加入列表的是record的引用，应该使用下面的式子来进行深复制，得到一个新的队列再加入列表。
                    record_list.append(deque(record))
                    num += 1
                else:
                    # 不是 最终状态则递归搜索
                    self.searchResult(record, bucket_volume, final_bucket_state)
                # 去除当前循环中添加的状态，进入下一个循环，关键步
                record.pop()


if __name__ == '__main__':
    s = Solution()
    # 水桶的初始状态
    initial_bucket_state = [0, 0, 8]
    # 每个水桶对应的容积
    bucket_volume = [3, 5, 8]
    num = 0
    record_list = []
    # 利用python的deque队列记录状态转移情况，
    # 初始化时加入水桶初始状态。deque是可以从头尾插入和删除的队列，在不指定大小时，为一个无边界的队列
    record = deque()
    record.append(initial_bucket_state)
    s.searchResult(record, bucket_volume, [0, 4, 4])
    print(min([len(i) for i in record_list]))
